...
bignum::bignum(const int64_t num) { SetDigit(num); }
bignum::bignum(CSTRX str) { SetDigit(str); }
bignum::bignum(const bignum& num) { SetDigit(num); }
...
bignum& bignum::SetDigit(CSTRX str) {
    Init();
    if (str.IsDigit()) {
        len_ = static_cast<int>(str.GetLength());
