...
uint32_t Hash_CRC32::GetCRC(uint32_t crcinit, uint8_t * bs, uint32_t bssize) {
    uint32_t crc = crcinit ^ 0xffffffff;
    while (bssize--)
        crc = (crc >> 8) ^ table[(crc & 0xff) ^ *bs++];
    return crc ^ 0xffffffff;
}

STRX Hash_CRC32::Generate(CSTRX dat) {
    string rec;
...
STRX Hash_CRC32::GenerateFile(CSTRX filename) {
